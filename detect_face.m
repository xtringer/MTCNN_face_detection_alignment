...
% 			im_data=imResample(im_data,[hs ws],'bilinear');
		...
		boxes=generateBoundingBox(out{2}(:,:,2),out{1},scales(j),threshold(1));
		%inter-scale nms
...
% 		score=squeeze(out{2}(2,:));
		...
		if numbox>0
			...
			tempimg=(tempimg-127.5)*0.0078125;%48*48*3*7
...
% 			score=squeeze(out{3}(2,:));
            ...
            points1(1:5,:)=repmat(w',[5 1]).*points(1:5,:)+repmat(total_boxes(:,1)',[5 1])-1;
            points1(6:10,:)=repmat(h',[5 1]).*points(6:10,:)+repmat(total_boxes(:,2)',[5 1])-1;
			if size(total_boxes,1)>0				
